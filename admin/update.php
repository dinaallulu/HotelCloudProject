...
if (isset($_POST['submit'])) {
   ...
   // Get and sanitize the name from the form
   $name = $_POST['name'];
   ...
   if (!empty($name)) {
      ...
      // If the username exists, display a warning message
   ...
   // Define an empty password hash value (sha1 of an empty string)
   $empty_pass = 'da39a3ee5e6b4b0d3255bfef95601890afd80709';
   // Get and sanitize the old, new, and confirm password inputs
   ...
   // Check if the old password is not the empty password hash
   if ($old_pass != $empty_pass) {
      ...
      if ($old_pass != $prev_pass) {
         ...
         if ($new_pass != $empty_pass) {
            ...
            // If the new password is empty, display a warning message
